...
package bitfade.ui.icons { 
	...
	public class Shape  {
		...
		// create the shaper
  		public static function create(type:String,size:uint=16,w:uint=16,color:int = -1):flash.display.Shape {
			...
			with (sh.graphics) {
				...
				switch (type) {
					...
					case "playlist":
						...
						var bStart:Number = boff+1
						
						var i:uint = 3
						
						while (i--) {
							moveTo(boff,bStart)
							lineTo(boff+bsize/6,bStart)
							moveTo(boff+bsize/6+4,bStart)
							lineTo(boff+bsize,bStart)
